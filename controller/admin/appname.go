...
func (this *AdminController) AdminAppname_indexAction() {
	this.Tplname = "views/admin/appname/index.html"
	/*tpl := make([]string, 0)
...
func (this *AdminController) AdminAppname_listAction() {
	...
	where := db.Tbname("app_name").Get_where_data(this.Postdata)
	...
	ct := db.Tbname("app_name").Where(where).Count()
	db.Dbinit()
	data := db.Tbname("app_name").Where(where).Order(order_str).Limit(limit_str).Select()
...
/* 获取当条记录Api */
func (this *AdminController) AdminAppname_editAction() {
	db := lib.NewQuerybuilder()
	...
	if (this.R.FormValue("id") !=""){
	    edit_flag="1"
		data = db.Tbname("app_name").Where(fmt.Sprintf("id=%v", this.R.FormValue("id"))).Find()
	} else {
		data = db.Tbname("app_name").Get_new_add()
	}
	this.Data["edit_flag"]=edit_flag
	this.Data["data"]=datatype.MapString2interface(data)
	this.Tplname = "views/admin/appname/edit.html"
	this.Rander()
}

/* 保存记录Api */
func (this *AdminController) AdminAppname_saveAction() {
	db := lib.NewQuerybuilder()
	msg := "保存失败"
	...
	if (this.R.FormValue("id") != "") {
		...
		_, err = db.Tbname("app_name").Where("id=" + Id).Update(this.Postdata)
	} else {
		_, err = db.Tbname("app_name").Insert(this.Postdata)
...
func (this *AdminController) AdminAppname_delAction() {
	db := lib.NewQuerybuilder()
	this.W.Header().Set("content-type", "application/json")
	jsonstr := new(Json_msg)
	jsonstr.Status = 101
	jsonstr.Msg = "删除失败"
	_, err := db.Tbname("app_name").Where(fmt.Sprintf("id in(%v)", this.R.FormValue("id"))).Delete()
	if(err!=nil){
	    jsonstr.Msg = "删除成功"
	    jsonstr.Status = 100
	}
