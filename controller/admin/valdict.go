...
func (this *AdminController) AdminValdict_indexAction() {
	this.Tplname = "views/admin/valdict/index.html"
	/*tpl := make([]string, 0)
...
func (this *AdminController) AdminValdict_listAction() {
	...
	where := db.Tbname("valdict").Get_where_data(this.Postdata)
	...
	ct := db.Tbname("valdict").Where(where).Count()
	db.Dbinit()
	data := db.Tbname("valdict").Where(where).Order(order_str).Limit(limit_str).Select()
...
/* 获取当条记录Api */
func (this *AdminController) AdminValdict_editAction() {
	db := lib.NewQuerybuilder()
	...
	if (this.R.FormValue("id") !=""){
	    edit_flag="1"
		data = db.Tbname("valdict").Where(fmt.Sprintf("id=%v", this.R.FormValue("id"))).Find()
	} else {
		data = db.Tbname("valdict").Get_new_add()
	}
	this.Data["edit_flag"]=edit_flag
	this.Data["data"]=datatype.MapString2interface(data)
	this.Tplname = "views/admin/valdict/edit.html"
	this.Rander()
}

/* 保存记录Api */
func (this *AdminController) AdminValdict_saveAction() {
	db := lib.NewQuerybuilder()
	msg := "保存失败"
	...
	if (this.R.FormValue("id") != "") {
		...
		_, err = db.Tbname("valdict").Where("id=" + Id).Update(this.Postdata)
	} else {
		_, err = db.Tbname("valdict").Insert(this.Postdata)
...
func (this *AdminController) AdminValdict_delAction() {
	db := lib.NewQuerybuilder()
	this.W.Header().Set("content-type", "application/json")
	jsonstr := new(Json_msg)
	jsonstr.Status = 101
	jsonstr.Msg = "删除失败"
	_, err := db.Tbname("valdict").Where(fmt.Sprintf("id in (%v)", this.R.FormValue("id"))).Delete()
	if(err==nil){
	    jsonstr.Msg = "删除成功"
	    jsonstr.Status = 100
	}
